<template>
                  ...
                  <slot name="body">
                    <div class="row">
                      <table id="tableDesign" class="table align-items-center mb-0">
                        ...
                        <tbody>
                          <tr v-if="tutors1.length == 0">
                            <td colspan="10"> No information available. </td>
                          </tr>
                          <tr v-else v-for="(tutor, index) in tutors1" :key="index">
  ...
  export default {
      ...
      methods: {
        eliminarDetalle(id){
          this.tutors1.splice(id, 1);
        },
        confirmarDetalle(id){
          this.tutors1[id].editing = false;
        },
        editarDetalle(id){
          this.tutors1[id].editing = true;
        },
        capitalizeFirstLetter(string) {
            return string.charAt(0).toUpperCase() + string.slice(1);
        },
        editTutor() {
          this.tutors1.forEach(item => {
            item.editing = false;
          });
