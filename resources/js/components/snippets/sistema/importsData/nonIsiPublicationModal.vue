...
export default {
      ...
      reader.onload = (e) => {
        ...
        if (jsonData.length > 0) {
          ...
          const expectedHeaders = [
            "idUsuario", "Status", "Progress Report", "Co-autoria centro", "Author(s)",
            "Number of Research Line", "Article Title", "Journal Name", "Volume", "First page",
    ...
    async processFile() {
      if (this.excelData.length > 0) {
        try {
          const response = await axios.post('api/importNonIsi', {
            data: this.excelData
          });
