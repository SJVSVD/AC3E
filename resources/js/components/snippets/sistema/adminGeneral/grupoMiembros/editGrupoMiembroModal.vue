                ...
                <div class="modal-wrapper">
                    <div class="modal-container-xs">
                        ...
                        <div class="modal-footer">
                            <slot name="footer">
                                <button class="btn btn-continue" @click="editarMiembro()" :disabled="buttonDisable">
                                    {{ buttonText }}
                                </button>
                            </slot>
                        </div>
...
export default {
    ...
    created(){
        this.id = this.grupoMiembro.id;
        this.idUsuario = this.grupoMiembro.idUsuario;
        this.idGrupo = this.grupoMiembro.idGrupo;
        this.getUsuarios();
        this.getGrupos();
    ...
    methods: {
        ...
        async editarMiembro() {
            const ok = await this.$refs.confirmation.show({
                ...
                message: `¿Está seguro/a que desea editar al miembro '${this.usuario.name}' del grupo '${this.grupo.nombre}'? Esta acción no puede ser desecha.`,
                okButton: 'Editar',
                cancelButton: 'Volver'
            })
            if (ok) {
                ...
                var id = this.id;
                axios.put(`../api/grupoMiembros/${id}`, grupoMiembro2).then((result) => {
                    this.buttonText = 'Guardando...';
                    this.buttonDisable = true;
                    this.toast.success("Miembro editada con éxito!", {
                        position: "top-right",
                        timeout: 3000,
                        closeOnClick: true,
