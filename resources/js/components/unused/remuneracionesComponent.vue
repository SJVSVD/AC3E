<template>
    <div class="row mt-4 mx-4">
        <div class="col-12">
                ...
                <div class="row">
                    <div class="col-9">
                        <div class="card-header pb-0">
                            <h6>Sistema de Remuneraciones</h6>
                        </div>
                    </div>
                    <div class="dropdown col-auto">
                        <div class="card-header pb-0">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                <li><a class="dropdown-item toggle-vis" data-column="1" href="#">Nombre</a></li>
                                <li><a class="dropdown-item toggle-vis" data-column="2" href="#">Correo Electrónico</a></li>
                                <li><a class="dropdown-item toggle-vis" data-column="3" href="#">Rol</a></li>
                                <li><a class="dropdown-item toggle-vis" data-column="4" href="#">Acciones</a></li>
                            </ul>
                        </div>
                        <p></p>
                    </div>
                    <div class="col-auto" style="margin-left: auto; margin-right: 60px;">
                        <div>
                            <a class="btn btn-continue" id="show-modal1" @click="showModal1 = true">Nuevo</a>
                        </div>
                    </div>
                </div>
                        ...
                        <table v-show="mostrarTabla" class="table align-items-center mb-0" id="MyTableRendiciones">
                            <thead>
                                <tr style="color: black">
                                    ...
                                    <th class="text-uppercase text-xs font-weight-bolder">Rol</th>
                            ...
                            <tbody>
                                <tr v-for="usuario in usuarios" :key="usuario.id">
                                    <td></td>
                                    <td>
                                        <p class="text-sm font-weight-bolder mb-0" style="color: black">{{ usuario.name }}</p>
                                    </td>
                                    <td>
                                        <p class="text-sm mb-0">{{ usuario.email }}</p>
                                    </td>
                                    <td>
                                        <p class="text-sm mb-0">{{ usuario.id }}</p>
                                    </td>
                                    <td class="align-middle text-end">
                                        <div class="d-flex px-3 py-1 justify-content-center align-items-center">
                                            <a class="btn btn-success btn-sm" title="Editar" @click="usuarioEditar(usuario)"><i class="fa fa-fw fa-edit"></i> Editar</a>
                                            &nbsp;
                                            <a class="btn btn-closed btn-sm" title="Eliminar" @click="deleteUsuario(usuario.id,usuario.name)"><i class="fa fa-fw fa-trash"></i>Eliminar</a>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
            ...
            <modaleditar v-bind:usuario="usuarioeditar" v-if="showModal2" @close="showModal2 = false"></modaleditar>
            <modalcrear v-if="showModal1" @close="showModal1 = false"></modalcrear>
        </div>
    </div>
</template>
    ...
    methods: {
        getUsuarios(){
            axios.get('api/usuarios').then( response =>{
                ...
                setTimeout(function(){
                    var table = $('#MyTableRemuneraciones').DataTable({
                        dom: 'Bfrtip',
                        buttons: [
